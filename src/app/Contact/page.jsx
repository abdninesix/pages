  ...
  return (
    <motion.div className="h-full" initial={{y:"-200vh"}} animate={{y:"0%"}} transition={{duration:1}}>
      <div className="h-full overflow-auto flex flex-col items-center justify-center gap-4 lg:gap-20 lg:flex-row pb-4 px-4 sm:px-8 md:px-12 lg:px-20 xl:px-48">
        ...
        <motion.form onSubmit={sendEmail} ref={form} className="shadow-md shadow-gray-800 bg-gray-500 text-white p-4 rounded-2xl h-2/3 w-[350px] lg:h-[550px] md:w-[500px] lg:mt-5 text-xl flex flex-col gap-5 justify-center" initial={{opacity:0, y:"30vh"}} animate={{opacity:1, y:"0%"}} transition={{delay:1, duration:0.5}}>
            <span>Your message:</span>
            <textarea rows={8} name="user_message" placeholder="Dear Abdullah," className="p-2 rounded-xl bg-gray-200 text-black border border-white resize-none"/>
            {errors.user_message && <span className="text-red-700">{errors.user_message}</span>}
            <span>Your email:</span>
            <input name="user_email" type="text" placeholder="something@somthing.com" className="p-2 rounded-xl bg-gray-200 text-black border border-white resize-none"/>
