  ...
  return (
    ...
    <motion.div className="flex flex-col gap-12 md:gap-20 h-full overflow-hidden" initial={{ y: "-200vh" }} animate={{ y: "0%" }} transition={{ duration: 1 }}>
      ...
      <div className="items-center flex flex-col gap-4 md:gap-10 lg:flex-row px-4 sm:px-8 md:px-12 lg:px-20 xl:px-40">
        ...
        <div className="h-auto lg:h-fit flex flex-col gap-5 lg:gap-8 justify-center text-center lg:text-left">

          <motion.div className="flex flex-col gap-4 lg:gap-8" initial={{ y: -50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 0.25, delay: 1.5, ease: "easeInOut" }}>
            <h1 className="text-4xl md:text-6xl font-bold">Meet <span className="text-mytheme">Abdullah</span></h1>
            <h1 className="text-2xl md:text-4xl font-bold duration-200 relative group">
              ...
              <span className="p-1 rounded-lg bg-gray-200 text-red-500">Laravel</span>&nbsp;Developer
